...
world = World.new { |w| w.time_step = 0.1 }
...
while world.clock <= 5 do

  world.run
  c_data << [world.clock, c.x]
  cc_data << [world.clock, cc.x]
  state_data << [world.clock, c.state.name.to_s[-1..-1].to_i * 1000]
  if world.clock == 1.6
    puts c.state.name
    puts cc.state.name
  end
